/* build Hessian vector product for Newton-CG algorithm
 *------------------------------------------------------------------------
 *
...
void build_Hv(emf_t *emf, fwi_t *fwi)
{
  ...
  /* 	  if(emf->nugrid){ */
  /* 	    if(emf->rd3==1){ */
  /* 	      fwd_emf = emf->u3s[j3][0]*emf->fwd_E3[ifreq][j3-1][j2][j1]/emf->inveps33[j3-1][j2][j1] */
  /* 		+ emf->u3s[j3][1]*emf->fwd_E3[ifreq][j3][j2][j1]/emf->inveps33[j3][j2][j1]; */
  /* 	      adj_emf = emf->u3s[j3][0]*emf->adj_E3[ifreq][j3-1][j2][j1]/emf->inveps33[j3-1][j2][j1] */
  /* 		+ emf->u3s[j3][1]*emf->adj_E3[ifreq][j3][j2][j1]/emf->inveps33[j3][j2][j1]; */
  /* 	    }else if(emf->rd3==2){ */
  /* 	      fwd_emf = emf->u3s[j3][0]*emf->fwd_E3[ifreq][j3-2][j2][j1]/emf->inveps33[j3-2][j2][j1] */
  /* 		+ emf->u3s[j3][1]*emf->fwd_E3[ifreq][j3-1][j2][j1]/emf->inveps33[j3-1][j2][j1] */
  /* 		+ emf->u3s[j3][2]*emf->fwd_E3[ifreq][j3][j2][j1]/emf->inveps33[j3][j2][j1] */
  /* 		+ emf->u3s[j3][3]*emf->fwd_E3[ifreq][j3+1][j2][j1]/emf->inveps33[j3+1][j2][j1]; */
  /* 	      adj_emf = emf->u3s[j3][0]*emf->adj_E3[ifreq][j3-2][j2][j1]/emf->inveps33[j3-2][j2][j1] */
  /* 		+ emf->u3s[j3][1]*emf->adj_E3[ifreq][j3-1][j2][j1]/emf->inveps33[j3-1][j2][j1] */
  /* 		+ emf->u3s[j3][2]*emf->adj_E3[ifreq][j3][j2][j1]/emf->inveps33[j3][j2][j1] */
  /* 		+ emf->u3s[j3][3]*emf->adj_E3[ifreq][j3+1][j2][j1]/emf->inveps33[j3+1][j2][j1]; */
  /* 	    } */
  /* 	  }else{ */
  /* 	    if(emf->rd3==1){ */
  ...
  /* 	tmp *= fwi->alpha;//multiply global scaling factor */
  /* 	fwi->Hv[i3][i2][i1] = tmp/emf->rho[i3][i2][i1];//dJ/d(log(rho))=<E^a|E^f>/rho */
  /*     } */
  /*   } */
  /* } */
