...
/*< read acquisition file to initialize survey geometry >*/
{
  ...
  if(!getparfloat("lensrc", &acqui->lensrc)) acqui->lensrc=275.;
  ...
  printf("isrc=%d nrec=%d\n", acqui->shot_idx[iproc], acqui->nrec);
  ...
  if(emf->verb){
    printf("lensrc=%g, nsubsrc=%d\n", acqui->lensrc, acqui->nsubsrc);
    printf("lenrec=%g, nsubrec=%d\n", acqui->lenrec, acqui->nsubrec);
